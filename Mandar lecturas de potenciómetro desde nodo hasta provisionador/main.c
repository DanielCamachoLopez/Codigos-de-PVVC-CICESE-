                                               ...
                                               esp_ble_mesh_generic_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT: 
    {
        ...
        // Detectar mensajes level del potenciómetro
        if (opcode == ESP_BLE_MESH_MODEL_OP_GEN_LEVEL_STATUS) {
            //  Obtiene el valor actual del potenciómetro enviado por el nodo
            int16_t level_value = param->status_cb.level_status.present_level;
            
            // Convertir de rango Level (0-1000) a porcentaje (0-100)
            uint8_t porcentaje = (level_value * 100) / 1000;
            
            // Imprime en el log el porcentaje y el valor bruto recibido del potenciómetro
            ESP_LOGI(TAG, "VALOR POTENCIÓMETRO: %d%% (Level: %d)", porcentaje, level_value);
        }
...
void app_main(void)
{
    ...
    while (1) {
        int boton = gpio_get_level(4);
        if (boton == 0) {
            ESP_LOGI("BOTON", "¡Botón presionado!");

            if (led_state == 0) {
                send_on_to_all_nodes();
                led_state = 1;
            } else {
                send_off_to_all_nodes();
                led_state = 0;
            }
