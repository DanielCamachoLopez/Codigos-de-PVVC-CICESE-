/* main.c - Application main entry point - PROVISIONADOR */
...
//Declara la esctructura del cliente que recibe los valores del potenciómetro de los nodos esclavos
static esp_ble_mesh_client_t level_client;

static esp_ble_mesh_model_t root_models[] = {
    ESP_BLE_MESH_MODEL_CFG_SRV(&config_server), // Permite que otros nodos lean/configuren este nodo
    ESP_BLE_MESH_MODEL_CFG_CLI(&config_client), // Permite enviar mensajes de configuración a nodos esclavos
    ESP_BLE_MESH_MODEL_GEN_ONOFF_CLI(NULL, &onoff_client), // Controla y recibe estado ON/OFF de los nodos
    ESP_BLE_MESH_MODEL_GEN_LEVEL_CLI(NULL, &level_client), // Recibe el nivel del potenciómetro de los nodos esclavos
};
                                               ...
                                               esp_ble_mesh_generic_client_cb_param_t *param)
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Este evento se dispara cuando un nodo esclavo publica un mensaje que el provisionador que debe recibir
    case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT: 
    {
        uint16_t addr = param->params->ctx.addr; // Se extrae la dirección unicast del nodo que envió el mensaje
        
        // Imprime un log indicando qué nodo envió el mensaje y qué tipo de mensaje
        // Opcode: 0x8204 (el opcode identifica si es Level, OnOff)
        ESP_LOGI(TAG, "MENSAJE RECIBIDO - Nodo: 0x%04x, Opcode: 0x%06" PRIx32, addr, opcode);
        
        // Detectar mensajes level del BMP180
        if (opcode == ESP_BLE_MESH_MODEL_OP_GEN_LEVEL_STATUS) {
            // Obtiene el valor de presión enviado por el nodo (en hPa)
            int16_t presion_hpa = param->status_cb.level_status.present_level;
            // Temperatura estimada
            float temperatura_estimada = (20.0 + ((presion_hpa - 844) * 0.8)) / 10.0f;
            
            // Logs para mostrar los datos del BMP180
            ESP_LOGI(TAG, "--- DATOS BMP180 RECIBIDOS ---");
            ESP_LOGI(TAG, "Nodo: 0x%04x", addr);
            ESP_LOGI(TAG, "Presión: %d hPa", presion_hpa);
            ESP_LOGI(TAG, "Temp. estimada: %.1f °C", temperatura_estimada);
            ESP_LOGI(TAG, "---------------------------------");
        }
        // Si el mensaje no es Level, puede ser un mensaje OnOff
        else if (opcode == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS) {
            // Obtiene el estado actual del nodo: 0 = OFF, 1 = ON
            uint8_t onoff_value = param->status_cb.onoff_status.present_onoff;

            // Muestra en el log si el nodo está encendido o apagado
            ESP_LOGI(TAG, "ESTADO ON/OFF: %s", onoff_value ? "ON" : "OFF");
        }
    }
    break;
...
static esp_err_t ble_mesh_init(void)
{
    ...
    esp_ble_mesh_register_generic_client_callback(example_ble_mesh_generic_client_cb); // Callback donde esta la lógica del sensor
...
static void send_on_to_all_nodes(void)
{
    for (int i = 0; i < ARRAY_SIZE(nodes); i++) {
        if (ESP_BLE_MESH_ADDR_IS_UNICAST(nodes[i].unicast)) {
            esp_ble_mesh_node_info_t *node = &nodes[i];
            esp_ble_mesh_client_common_param_t common = {0};
            esp_err_t err = example_ble_mesh_set_msg_common(&common, node, onoff_client.model, ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET);
            if (err) {
                ESP_LOGE(TAG, "Failed to set common param for node 0x%04x", node->unicast);
                continue;
            }

            esp_ble_mesh_generic_client_set_state_t set_state = {0};
            set_state.onoff_set.op_en = false;
            set_state.onoff_set.onoff = LED_ON;
            set_state.onoff_set.tid = 0;
            err = esp_ble_mesh_generic_client_set_state(&common, &set_state);
            if (err) {
                ESP_LOGE(TAG, "Failed to send ON to node 0x%04x", node->unicast);
            } else {
                ESP_LOGI(TAG, "Sent ON to node 0x%04x", node->unicast);
            }
        }
    }
}

static void send_off_to_all_nodes(void)
{
    for (int i = 0; i < ARRAY_SIZE(nodes); i++) {
        if (ESP_BLE_MESH_ADDR_IS_UNICAST(nodes[i].unicast)) {
            esp_ble_mesh_node_info_t *node = &nodes[i];
            ...
            esp_ble_mesh_generic_client_set_state_t set_state = {0};
            set_state.onoff_set.op_en = false;
            set_state.onoff_set.onoff = LED_OFF;
            set_state.onoff_set.tid = 0;
            err = esp_ble_mesh_generic_client_set_state(&common, &set_state);
            if (err) {
                ESP_LOGE(TAG, "Failed to send OFF to node 0x%04x", node->unicast);
            } else {
                ESP_LOGI(TAG, "Sent OFF to node 0x%04x", node->unicast);
            }
        }
    }
}
...
void app_main(void)
{
    esp_err_t err; // Almacena los errores

    ESP_LOGI(TAG, "Initializing..."); // Indica que se está inicializando todo

    err = nvs_flash_init(); // Inicializa la memoria no volátil que guarda configuraciones persistentes
    if (err == ESP_ERR_NVS_NO_FREE_PAGES) { // Si la memoria no volátil está llena, se borra y se reinicializa
        ESP_ERROR_CHECK(nvs_flash_erase());
        err = nvs_flash_init();
    }
    ESP_ERROR_CHECK(err); // Si hubo error se detiene el programa y muestra el log del error

    err = bluetooth_init(); // Inicializa el stack de Bluetooth (cosas básicas del controlador BLE, la pila de protocolos, y el modo Mesh)
    if (err) {
        ESP_LOGE(TAG, "esp32_bluetooth_init failed (err %d)", err); // Si hubo error detiene el programa y muestra el log
        return;
    }

    // Obtiene el UUID único del dispositivo
    ble_mesh_get_dev_uuid(dev_uuid); // Es esencial para que el provisionador y los nodos se reconozcan

    // Inicializa el sistema Bluetooth Mesh completo
    err = ble_mesh_init();
    if (err) {
        ESP_LOGE(TAG, "Bluetooth mesh init failed (err %d)", err); // Si hubo error detiene el programa y muestra el log
    }

    // Configuración física del botón en el GPIO4
    ...
    gpio_config(&btn_conf); // Aplica esas configuraciones en el esp32

    static uint8_t led_state = 0; // Crea variable para guardar estado del LED

    while (1) {
        int boton = gpio_get_level(4); // Lee el valor del pin 4
        if (boton == 0) { // Si está presionado
            ESP_LOGI("BOTON", "¡Botón presionado!"); // Indica que está presionado

            if (led_state == 0) {
                send_on_to_all_nodes(); // Si está presionado manda el comando ON a todos los nodos
                led_state = 1; // Actualiza la variable de estado
            } else {
                send_off_to_all_nodes(); // Si no está presionado manda el comando OFF a todos los nodos
                led_state = 0; // Actualiza la variable de estado
            }

            vTaskDelay(300 / portTICK_PERIOD_MS); // Espera un momento para evitar rebotes
        }
        vTaskDelay(50 / portTICK_PERIOD_MS); // Espera 50 ms entre lecturas para no saturar la CPU
    }
}
