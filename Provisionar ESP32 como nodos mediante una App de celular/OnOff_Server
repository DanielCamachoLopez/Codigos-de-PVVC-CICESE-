...
struct _led_state {
    uint8_t pin;
    uint8_t current; // 0 = apagado, 1 = encendido
};

// Inicializa tres LEDs conectados a los GPIO 2, 4 y 5
struct _led_state led_state[3] = {
    { .pin = 2, .current = 0 },  // LED 0
    { .pin = 4, .current = 0 },  // LED 1
    { .pin = 5, .current = 0 }   // LED 2
};
...
void app_main(void)
{
    ...
    // Inicializar LEDs físicos como salida y apagados al inicio
    for (int i = 0; i < 3; i++) {
        gpio_reset_pin(led_state[i].pin);
        gpio_set_direction(led_state[i].pin, GPIO_MODE_OUTPUT);
        gpio_set_level(led_state[i].pin, 0); // Apagado
    }
